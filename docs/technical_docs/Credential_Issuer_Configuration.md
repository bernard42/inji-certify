...
- **Per-configuration selection of `cryptographicBindingMethodsSupported`, `credentialSigningAlgValuesSupported` and `proofTypesSupported`**.
  ...
  - For the `mso_mdoc` format the algorithms are advertised as COSE identifiers, so every selected value must have a COSE equivalent — `ES256`, `EdDSA`, `ES256K` or `RS256`. A value declared for the crypto suite but without one is rejected with `unsupported_credential_signing_alg`.
